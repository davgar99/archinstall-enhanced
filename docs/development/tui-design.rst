...
Every choice prompt opens with the cursor on the first list item, for both
single-select and multi-select menus. Helpers do not call
``MenuItemGroup.set_focus_by_value`` to restore a previous choice or jump to a
recommended value; the recommended option is communicated with the ``(default)``
label instead, and multi-select prompts still show checkmarks on the items that
are already set. Confirmations follow the same rule: the group is always ``No``
then ``Yes`` with ``No`` focused. The persistent navigation menus built on
``AbstractMenu`` are the one exception and keep their cursor position across
round-trips.
