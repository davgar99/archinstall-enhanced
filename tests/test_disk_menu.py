...
def test_lvm_menu_exposes_none_default_and_back(monkeypatch: MonkeyPatch) -> None:
	...
	# Even with an existing LVM preset the cursor starts on the safe first
	# option rather than pre-selecting the LVM layout.
	assert seen[0]._group.focus_item is not None
	assert seen[0]._group.focus_item.text == 'Do not use LVM'
