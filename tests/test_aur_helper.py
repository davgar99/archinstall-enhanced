...
def test_aur_helper_builds_as_configured_sudo_user(tmp_path: Path) -> None:
	...
	assert installer.commands == [
		('rm -rf -- /home/builder/.cache/archinstall-yay', None, False),
		('install -d -m 0700 -o builder -- /home/builder/.cache/archinstall-yay', None, False),
		('git clone --depth=1 https://aur.archlinux.org/yay.git /home/builder/.cache/archinstall-yay', 'builder', True),
		('cd /home/builder/.cache/archinstall-yay && makepkg --syncdeps --install --needed --noconfirm', 'builder', True),
	]
	expected_sudoers = (
...
def test_aura_uses_source_package(tmp_path: Path) -> None:
	...
	assert installer.commands[2] == (
		'git clone --depth=1 https://aur.archlinux.org/aura.git /home/builder/.cache/archinstall-aura',
		'builder',
		True,
	)
...
def test_aur_helper_cleans_stale_build_directory_before_clone(tmp_path: Path) -> None:
	...
	assert installer.commands[0] == ('rm -rf -- /home/builder/.cache/archinstall-pikaur', None, False)
	assert installer.commands[1] == ('install -d -m 0700 -o builder -- /home/builder/.cache/archinstall-pikaur', None, False)
